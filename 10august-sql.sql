...
CREATE TABLE employee_performances ( employee_id INT, employee_name VARCHAR(50), 
department VARCHAR(30), location VARCHAR(30), performance_year INT, performance_month INT,
salary DECIMAL(10,2), sales_amount DECIMAL(12,2), rating DECIMAL(3,1)); 
 ...
 select department, sum(sales_amount) from employee_performances; -- will give error
 
 select *, sum(sales_amount) over() from employee_performances;

-- total sales per location 
select *, sum(sales_amount) over(partition by location) from employee_performances;
 
 -- order by --> orders data in ascending or descending order 
 -- cumulative sum of sales
 select *, sum(sales_amount) over(order by sales_amount) from employee_performances;
 
 select employee_id, employee_name, location, sales_amount, sum(sales_amount) over(), 
 sum(sales_amount) over(partition by location),
 sum(sales_amount) over( partition by location order by sales_amount) from employee_performances;
